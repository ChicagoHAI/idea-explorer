...
def get_cohere_client(
    ...
    if cohere_api_key is not None:
        ...
        logger.warning("Cohere API key not found - reranking disabled (scores will be 0.0)")
        return None
...
class NoOpRerankScorer(RerankScorer):
    """No-op reranker that returns 0.0 scores when Cohere is not configured."""

    async def rerank(self, rerank_input: RerankScoreInput) -> RerankScoreOutput:
        return RerankScoreOutput(
            results=[RerankScoreResult(corpus_id=d.corpus_id, score=0.0) for d in rerank_input.docs],
            method="noop",
        )
